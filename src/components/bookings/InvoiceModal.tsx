...
export const InvoiceModal: React.FC<{
	open: boolean;
	handleClose: () => void;
	bookingTransaction: TransactionTypes[];
}> = ({ open, handleClose, bookingTransaction }) => {
	const InvoiceTableHeaders = [
		{
			title: 'Amount',
			render: (row: TransactionTypes) => `₦${numberWithCommas(row.amount)}`,
		},
		{ title: 'Status', render: (row: TransactionTypes) => `${row.status}` },
		{
			title: 'Narration',
			render: (row: TransactionTypes) => `${row.narration}`,
		},
	];
	return (
		<div>
