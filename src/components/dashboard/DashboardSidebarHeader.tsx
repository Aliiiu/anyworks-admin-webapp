...
const DashboardSidebarHeaderContainer = styled.div`
  ...
  visibility: hidden;
  width: auto;
    min-width: 150px;  color: ${(props) => props.theme.colors.text_01};
  background-color: ${(props) => props.theme.colors.white};
  text-align: center;
  border-radius: 6px;
  padding: 5px;
...
export const DashboardSidebarHeader  : React.FC<Props> =({ isOpen, toggleSidebar, showOnDesktop }) => {


  return (
    <DashboardSidebarHeaderContainer className={clsx('DashboardSidebarHeader', { showOnDesktop })}>
     <Link to='/dashboard'>
     <img className='brand-logo' src={logo} alt="anyworks" /></Link>
      <div className="tooltip__avatar">
      <img className='showOnMobile' src={avatar} alt="avatar" />
      <div className="tooltiptext">  <p className="tooltiptext__user--name">
                    Olajide Olajide
                  </p>
