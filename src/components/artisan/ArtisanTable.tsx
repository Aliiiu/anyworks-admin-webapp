...
const ArtisanTable = () => {
  ...
  const handleCloseNotificationModal = () => setOpenSendNotificationModal(false)
  ...
  const sortedData = () => {
    if (selectedFilterValue === 'all') {
      ...
      return filteredData.filter((item: any) => item.status.toLowerCase().includes('offline'))
    } else if (selectedFilterValue === 'online') {
      return filteredData.filter((item: any) => item.status.toLowerCase().includes('online'))
    } else {
      return filteredData
    }
  }
  ...
  const ArtisanTableHeaders = [
    ...
    {
      title: 'DP',
      render: (row: any) => <img style={{ width: '40px' }} src={row.img} alt="" />,
    },
    {
      title: 'Gender',
  ...
  return (
    ...
    >
      <ArtisanTableContainer>
        ...
        <SendMailModal open={openSendMailModal} handleClose={handleCloseMailModal} />
